...
public class getLinks
{
    ...
    final static String oid = "1.3.6.1.2.1.17.4.3.1.1";

    private SnmpContext context;
    private BlockPdu pdu;
    private static Vector macs;
    private Node sNode; 

public getLinks(Node aNode)
{
 sNode = aNode;	
 macs = new Vector();
}
    ...
    try
    {
        while (running)
        {
            ...
            if (pdu.getErrorStatus() == AsnObject.SNMP_ERR_NOERROR)
            {
                ...
                if (obj != null
                        &&
                    obj.getRespType() != AsnObject.SNMP_VAR_ENDOFMIBVIEW && obj.getRespTypeString().equals("ASN_OCTET_STR"))
                {
                    AsnObjectId oid = var.getOid();
                    String mac = var.getValue().toString().replaceAll("0x","");
...
public static Vector getConnectedNodes(Node aNode)
{

    getLinks application = new getLinks(aNode);
    application.init();
    application.start();
    return macs;
}
