...
public final class IPAddressRange {
	...
	static class IPAddressRangeGenerator implements Enumeration, Iterator {
		...
		public void remove() {
			throw new UnsupportedOperationException(
					"The remove operation is not supported by the iterator");
		}
	...
	IPAddressRange(String fromIP, String toIP)
		...
		m_begin = IPSorter.convertToLong(InetAddress.getByName(fromIP)
				.getAddress());
		m_end = IPSorter
				.convertToLong(InetAddress.getByName(toIP).getAddress());
	...
	boolean contains(String ipAddr) throws java.net.UnknownHostException {
		return contains(convertToLong(InetAddress.getByName(ipAddr)
				.getAddress()));
	}
	...
	public static long convertToLong(byte[] addr) {
		if (addr == null) {
			throw new IllegalArgumentException(
					"The passed array must not be null");
		}
