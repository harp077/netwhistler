...
public class StatusPanel extends JPanel {
	...
	public StatusPanel() {
		...
		model.addColumn("UpTime");
		model.addColumn("DownTime");
		model.addColumn("SNMP");
		...
		for (int i = 0; i < model.getRowCount(); i++) {
			...
			if (ip.equals(aNode.getIP())) {
				...
				if (NodeStat.equals("DOWN"))
					...
					Status = NodeStat;
				String response = aNode.getResponse();
				if (!NetworkManagerGUI.textHasContent(response))
					response = "n/a";
				String Services = "";
				if (!NodeStat.equals("DOWN")) {
				...
				String DownTime = pingTimer.millisecondsToString(aNode
						.getDownTime());
				...
				model.setValueAt(aNode.getnodeType(), i, 3);
				model.setValueAt(NodeMon, i, 4);
				model.setValueAt("<html>" + Status, i, 5);
				model.setValueAt(response, i, 6);
				model.setValueAt("<html>" + Services, i, 7);
				model.setValueAt(UpTime, i, 8);
				model.setValueAt(DownTime, i, 9);
				model.setValueAt(NodeSNMP, i, 10);
			}
		}
	...
	public static void addNode(Node aNode) {
		...
		model.insertRow(model.getRowCount(), new Object[] { num, aNode.getIP(),
				aNode.getDNSname(), aNode.getnodeType(), NodeMon,
				"<html>" + Status, response, "<html>" + Services, UpTime, DownTime,
				NodeSNMP });
	}
